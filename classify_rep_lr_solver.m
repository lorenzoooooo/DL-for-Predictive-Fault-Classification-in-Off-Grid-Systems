...
% default value).
solvername=["adam";"rmsprop";"sgdm"];
lr=[0.001:0.001:0.01];
for i=1:size(lr,2)
    for j=1:size(solvername,1)
        close all force;
        options = trainingOptions(solvername(j), ...
            'InitialLearnRate', lr(i), ...
        ...
        file=strcat(strcat(solvername(j),'_',string(day(datetime)),'-',string(month(datetime)),'_',string(numHiddenUnits),'_',string(options.InitialLearnRate),'_',string(round(acc,2))));
