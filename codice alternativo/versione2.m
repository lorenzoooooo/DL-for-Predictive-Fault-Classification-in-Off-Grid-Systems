function [dati, bug, bug_idx]  = versione2(temp_data,var)
global torre tipo
col=2;
counter=1;
dati=temp_data(:,1);
...
while col<size(temp_data,2)                                 %scelgo la colonna
    if temp_data(1,col)==temp_data(1,col+1)
        counter=find(temp_data(1,col)==temp_data(1,:));     %conto quali colonne rappresentano lo stesso istante
    ...
    dati=[dati sottomatrice(:,1)];
end
fclose(fileID);
bug=temp_data;
bug_idx=cell2mat(bug_idx);